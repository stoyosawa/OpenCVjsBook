...
OpenCVには`cv.Text()`という関数があるので、そちらからでも文字列描画できます。描画コンテクスト経由でもどちらでもかまいません。例外は、日本語文字列を使いたいときで、これはOpenCVには日本語フォントが用意されていないからです。日本語フォントの使い方は[2.4節](#24-日本語文字の重畳とキーボード処理 "INTERNAL")で説明します。
...
プルダウンメニュー操作イベントの登録（41行目）を、ページの全要素を読み込んだときに実行される`imageLoaded()`関数（37～43行目）から行っているのは、[2.2節](#22-ビデオをキャプチャする "INTERNAL")で説明したローディングタイミングに対処するためです。
