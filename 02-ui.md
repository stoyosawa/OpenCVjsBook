...
OpenCVには`cv.Text()`という関数があるので、そちらからでも文字列描画できます。描画コンテクスト経由でもどちらでもかまいません。例外は、日本語文字列を使いたいときで、これはOpenCVには日本語フォントが用意されていないからです。日本語フォントの使い方は[2.4節](#24-日本語文字を画像に重畳する "INTERNAL")で説明します。
...
いくつかについては、その原理を[第5章](TBA "INTERNAL")で説明します。次に示すMDNのページには、`filter`で利用できる関数がリストされています。また、それぞれの関数の詳細説明ページへのリンクがあり、値と結果をサンプルから参照できるようになっています。
