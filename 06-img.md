...
本章では画像処理技術を応用して、選択した領域だけにモザイクをかける、画像からイラスト／アニメ風の絵を起こす、画像から線画を生成する、電線を消す、QRコードを読む、スキャンした書類の傾きを補正する、顔位置の検出する、画像レイヤーを合成するというスクリプトを紹介します。
...
画像の部分領域にモザイクをかけます。顔、住所や表札、商標といった公開したくない部分を見づらくする難読化（obfuscation）テクニックの1つです。

領域選択には、[2.7節](./02-ui.md#27-マウス操作で部分領域を切り取る "INTERNAL")で作成した`RegionSelect`クラスを使います。モザイクの度合いは、[2.6節](./02-ui.md#26-トラックバーからサイズを調整する "INTERNAL")で説明したトラックバーから選択します。

<!-- 補間/内挿 は [a, ..., z] の間を埋める、補間/外挿は [a, ..., z] の外側を埋める。適当に埋める（規定値で埋めるなど）は補完にした。 -->
技術的には、`cv.resize()`関数を使ったリサイズ操作です。画像を拡大しようと引っ張り伸ばすと、ピクセルの間に隙間ができます。もっともシンプルな方法でこの隙間を埋めると結果としてモザイク状になるという特徴を利用するわけです。画像の一部だけをリサイズするには[4.7節](./04-mat.md#47-部分領域だけ処理する "INTERNAL")のROIを使います。
...
左が横幅36ピクセルに縮小表示した元画像です。右がこれを10倍に拡大したときのものです。

リサイズの補間方法はプルダウンメニューから選択でき、この画面では12個が用意されています。[3.5節](./03-opencv.md#35-OpenCVjsの定数を調べる "INTERNAL")と同じ方法で`cv`オブジェクトから無批判に抽出しただけなので、`cv.resize()`では利用できないものもあります。その場合はコンソールにその旨表示され、選択は無視されます。ここで選択したのは`cv.INTER_LINEAR`という方法で、ぼけた画像になります。最小値は2、最大値は30、デフォルトは10にしています。
...
縮小率はモザイクの大きさを示しています。コードは画像幅と高さをここで指定した値でいったん割って（縮小）、それからもとに戻すことでモザイク化しています。最小値は2、最大値は30です。200×200の領域を指定すると、デフォルト値の10のときは20×20に縮小されるので、モザイクのタイル数は縦横20個ずつになります。縮小率が大きいほどタイルが大きくなります。
...
25行目は[3.5節](./03-opencv.md#35-OpenCVjsの定数を調べる "INTERNAL")で使った手で、補間方法定数名がINTER_で始まることを利用しています。すべてが必ずしも`cv.resize()`で使えるとはかぎらないのに加え、他にも`WARP_`で始まる定数が2つ利用可能だったりするので、詳細はリファレンスを参照してください。検索フィールドから「InterpolationFlags」を入力すれば見つかります。
...
`cv.INTER_LINEAR` | 1 | バイリニア補間（デフォルト）。両端の2点（バイ）の間を一次関数（リニア）で補間する（画像は2次元なので縦横両方）。処理は早い。
...
第4引数`fx`と第5引数`fy`には横と縦の倍率を指定します。1より大きければ拡大、1より小さければ縮小です。デフォルトの0なときは、第3引数の`dsize`が用いられます。0以外が指定されれば、第3引数は無視されます。
...
[File] img-resize2.html
 ...
 18  </div>
 19  <div>
 20    縮小率 <input type="range" id="rangeTag" min="2" value="10" max="30"/>
 21  </div>
 22
 23  <script>
 ...
 44      cv.resize(mat, mat, new cv.Size(region[2], region[3]),
 45        0, 0, cv.INTER_NEAREST);
 46      mat.copyTo(roi);
 47      cv.imshow('canvasTagMosaic', mat);
 48      cv.imshow('canvasTag', src);
 49
 50      [src, roi].forEach(m => m.delete());
 51    }
 52
 53    function resourceReady() {
 54      readyFlag |= 1;
 55      let rs = new RegionSelect('imageTag');
 56      imgElem.addEventListener('regionselect', imgProc);
 57      rangeElem.addEventListener('input', imgProc);
 58    }
 59
 60    function opencvReady() {
 61      readyFlag |= 2;
 62      rangeElem.dispatchEvent(new InputEvent('input'));
 63    }
 64
 65    window.addEventListener('load', resourceReady);
 66    var Module = {
 67      onRuntimeInitialized: opencvReady
 68    }
 69  </script>
 70
 71  </body>
...
続いて、このROIをトラックバー指定の値（41行目の`scale`）で縮小し（43行目）、またもとの大きさに拡大します（44～45行目）。拡大時には`cv.INTER_NEAREST`を使うことで、あえてモザイク状にします。
...
第2引数で出力を受ける`cv.Mat()`は必ず用意します（42行目）。

本書のコードでは、しばしば`cv.blur(src, src, ...)`のように、入力画像に出力を上書きしています。同じオブジェクトを使えば、無用に`cv.Mat`を増やさなくてよいからです（消し忘れも減ります）。しかし、`cv.resize()`ではこれはできません。出力のサイズが異なるので、同じところに上書きできないからです。

そのため、この第2引数の`mat`は元画像のROIとは別のオブジェクトです。そこで、縮小拡大後のこのブジェクトの中身は、明示的にROIに戻さなければなりません。これをやっているのが46行目です。

```javascript
 46      mat.copyTo(roi);
```

どのコードがどのタイミングで`cv.Mat`を再利用するのか新規生成をするのがわかりにくいことはしばしばあります。処理結果が反映されていないときは、対象のオブジェクトが同じか異なるかを確認してください。
...
画像をぼけさせます。これも難読化テクニックの1つですが、画像処理ではノイズや傷など細かく不要な要素を取り除くことで、続く別の処理の効果を高めるのによく用いられます。

これまで「ぼけ」といってきましたが、正確には平滑化（スムージング）処理と呼ばれます。しみやそばかすなどの皮膚のでこぼこを滑らかに周囲に溶け込ませることで目立たなくする、美肌効果があります。昔の映画でソフトフォーカスと呼ばれていた効果に似ています。

技術的には、4種類の平滑化関数`cv.blur()`、`cv.GaussianBlur()`、`cv.medianBlur()`、`cv.bilateralFilter()`を取り上げます。名称は順に平均化フィルタ、ガウス平滑化フィルタ、中間値フィルタ、バイラテラルフィルタです。どれにも「フィルタ」と付いているのは、これらがターゲットのピクセルを色眼鏡で見るかのように「フィルタをかけ」ることによって変更するからです。
関数にいくつもあるパラメータと効果の関連性を理解するため、ぼかしの理論も簡単に説明します。

実行例を次の画面に示します。

<img src="Images/Ch06/img-blur-1.png">

左側が元画像、右側が平滑化フィルタ適用後の画像です。ここでは平均化フィルタを用いています。効果がわかりやすくなるよう、あえてぼけの多くなるパラメータを用いています。

残りの3つのフィルタによる結果を次に示します。
...
左から順にガウス平滑化フィルタ、中間値フィルタ、バイラテラルフィルタのものです。ガウス平滑化フィルタは平均化のものとあまり変わりありません。中間値フィルタのものは、印象画のように物体単位で色合いがまとまった感じになります。対象のピクセルをまわりと混ぜ合わせるのがぼかしの基本なので、水彩絵具が混ざり合ったように輪郭のシャープさが失われるわけです。バイラテラルフィルタでは色合いのまとまりは中間値と似ていますが、輪郭はよりはっきりしています。
...
[File] img-blur.html
 ...
 18      <option value="blur">平均化フィルタ</option>
 19      <option value="gaussian">ガウス平滑化フィルタ</option>
...
左が画像を表す行列で、今、中心にあるピクセルを変換しようとしています。この中心を「注目ピクセル」、周辺の8つのピクセルを含めたエリアを「注目領域」といいます。その右にあるのは左と同じサイズの行列で、要素はすべて1/9です。
...
2番目の行列をフィルタ、あるいはカーネルと呼びます。

図のフィルタの要素はすべて1/9です。要素をすべて同じ値で乗じてから和を取るという操作は、和を取ってからその値で乗ずるのと同じ計算です。そして、1/9の9は注目領域のピクセルの数です。この畳み込み演算はつまり、注目領域の平均を計算し、その値で注目ピクセルを置き換える処理です。

フィルタのサイズを変更したり、要素の値を変化させることで、いろいろな変換が可能になります。

カラーやアルファチャネル付き画像では、各チャネルをそれぞれ独立したものとして操作します。ピクセルの入れ替えなので、入力と出力の画像のサイズもデータ型も同じです。

#### 平均化フィルタ－cv.blur

44行目の`cv.blur()`関数は、要素を要素数の逆数で埋めたフィルタを用いることで、注目領域内のピクセルの平均値で注目ピクセルを置き換えます。
...
一定の枠内の平均で値を置き換える操作は、一次元では移動平均です。株価や為替のチャートでよく見る移動平均では、平均区間を長くとると（1年など）カーブがスムーズになり、短くとると（1週間など）カーブがもとの形を反映してごつごつします。それと同じで、フィルタのサイズを大きくすれば画像がより平滑化され、小さくすればもとの画像に近くなります。
...
第4引数`anchor`には注目ピクセルの位置を、`cv.Point`オブジェクトから指定します。デフォルトの(-1, -1)のときはフィルタの中央で、凝ったことをしているのでなければデフォルトのまま使います。

第5引数`borderType`には画像端でのピクセルの推定方法を指定します。

#### cv.Pointオブジェクト

`cv.Point`は点の座標位置を示すオブジェクトで、[4.4節](./04-mat.md#44-モノクロで円を描く "INTERNAL")の`cv.Size`や[4.7節](./04-mat.md#47-部分領域だけ処理する "INTERNAL")の`cv.Rect`同様、幾何学的な情報を収容するJavaScriptのオブジェクトです。点のx位置とy位置を示す`x`、`y`のキーを持ちます。
...
(-1, -1)は未指定を示すときによく用いられます。

#### 画像端外挿方法

畳み込み演算には、注目ピクセルの周りに他のピクセルが必要です。しかし、次の図に示すように、画像端になると外部にはピクセルがありません。

<!-- 原画は ODG にあり。182 x 187 -->
...
ガウス関数は、グラフにすると次の図のように吊り鐘状になっています（偏差値でよく見る正規分布です）。

<!-- 原画は xlsx にあり。 1059 x 502 -->
<img src="Images/Ch06/img-blur-gaussian.png" width="500">

 $\sigma$ の値が小さくなればなるほど吊り鐘はとがり、したがって注目ピクセルから外れたところの寄与が減ります。つまり、ぼけが少なくなります。反対に、 $\sigma$ の値が大きくなればなるほど吊り鐘はフラットになり、遠方のピクセルが結果に寄与する度合いが大きくなります。これはぼけを増やす方向に働きます。
...
第1引数`src`、第2引数`dst`、第3引数`ksize`、第6引数`borderType`は`cv.blur()`と同じです。`ksize`が大きくなればぼけが増えるのも同じです。

第4引数`sigmaX`はx方向の、第5引数`sigmaY`はy方向のそれぞれ標準偏差です。`sigmaY`にデフォルトの0を指定すると、`sigmaX`と同じ値が取られます。どちらも0にすると、`ksize`から適当に決められます。

#### 中間値フィルタ－cv.medianBlur
...
中間値は値を順番に並べ、ちょうど中間に位置する値を採用する統計的な測度です。しばしば、給与や資産など偏りがいびつで、平均があまり意味をなさないデータの統計で用いられます。英語でメディアンなので、メディアンフィルタとも呼ばれます。

平均値フィルタとガウス平滑化フィルタは正方行列で表現できますが、中間値フィルタは並べ替えを伴うので畳み込みでは計算できません。線形代数では解けないので、非線形フィルターと呼ばれます。
...
引数はいずれもこれまでと同じです。フィルタ内部の値だけで計算が完結するので、ピクセル外挿は必要ありません。
...
#### バイラテラルフィルタ－cv.bilateralFilter

53行目の`cv.bilateralFilter()`関数はバイラテラルフィルタという、ガウス関数をもう一段複雑にした関数を用います。バイラテラルという英語には、両面や両側や互恵的といった訳語が与えられています。

```javascript
 53        cv.bilateralFilter(src, dst, ksize, 75, 75, cv.BORDER_DEFAULT);
```

ガウス平滑化フィルタでは、注目ピクセルからの距離が近いか遠いかによって重み付けを変えて平均を取りました。バイラテラルはこれに加えて、注目ピクセルの値に近いか遠いかにも着目します。距離を相手にしている前者を空間方向、ピクセル値を考慮する後者を色方向といいます。

関数の形はガウス関数の指数関数を2つ乗じた格好になっています。このフィルタで輪郭が出やすくなるのは、色方向の距離を考慮しているからです。輪郭は前景と背景、あるいは2つの物体や色の間の仮想的な線です（アニメや漫画ではないので、実世界の物体の輪郭には線は描かれていません）。したがって、輪郭をはさんだピクセル値はそれぞれの物体の中よりも大きく離れています。バイラテラルフィルタは、その差が大きければ値を温存する方向に動きます。
...
cv.bilateralFilter(                         // 戻り値なし
    ...
    cv.Mat dst,                             // 出力画像
    cv.Size ksize,                          // フィルタのサイズ
    number sigmaColor,                      // 空間方向の標準偏差
    number sigmaSpace,                      // 空間方向の標準偏差
    number borderType = cv.BORDER_DEFAULT   // 画像端でのピクセルの外挿方法

);
```

第4引数`sigmaColor`が色方向用の`sigmaColor`、第5引数`sigmaSpace`が空間方向用の標準偏差です。これら以外の引数は`cv.GaussianBlur()`と同じです。
...
リファレンスは標準偏差に10未満の値を指定しても大して効果はなく、150より大きいと効果が絶大すぎてアニメ絵に近くなると述べています。コードで用いている75という値は、OpenCVチュートリアルからそのまま取ってきたものです。

参考までに、どちらの標準偏差も50、75（再掲）、150にしたときの結果を次に左から順に示します。
...
技術的には、線画は画像の「エッジ」（輪郭）を検出することで得られます。この処理は、微分を施すフィルタで畳み込み演算を施すことで行われます。

本節ではコードを2つに分けて説明します。第1のコードでは、OpenCVの微分フィルタ関数`cv.Sobel()`、`cv.Laplacian()`、`cv.Canny()`を適用し、それぞれのできばえを比較します。第2のコードでは輪郭付きアニメ絵風を生成します。
...
左が元画像、中央がキャニーの結果の白黒を反転させることで輪郭線を黒にしたものです。右が輪郭付きアニメ絵風で、元画像を`cv.bilateralFilter()`で処理し、それを中央の画像をマスクとして背景灰色の画像に貼り付けています（マスクについては[5.8節](./05-colors.md#58-背景を入れ替える輝度調整付き "INTERNAL")参照）。

#### エッジ検出の原理

画像のエッジ（輪郭線）は、背景と前景の境界線でピクセル値が大きく変化するという仮定をもとに抽出されます。

エッジ検出の原理を、白い前景がグレーの背景に写っている次の模式的な画像から考えます。データ型は1チャネル8ビット符号なし整数（`cv.CV_8UC1`）としているので、ピクセル値の範囲は0～255です。
...
第7引数`borderType`に画像外郭のピクセル値を外挿する方法を定数から指定します。外挿方法は[6.2節](#62-画像をぼかす "INTERNAL")で説明しました。
...
2. ソベルフィルタをxとy方向にかけます。第5引数`apartureSize`がこのときのフィルタサイズです。`cv.Sobel()`のフィルタサイズには1～7の奇数のみしか指定できないので、ここでもこの制約が適用されます。
...
Cannyアルゴリズムは、OpenCVチュートリアルの「Canny Edge Detection」に詳しく説明されています。次のURLから参照してください。
...
③単一色で染めた、元画像と同じサイズの画像を用意します（32行目）。この色が線色になります。ここでは(50, 50, 50）のやや濃い目のグレーを用いています。
...
QRコードを読み取ります。背景に埋もれていても、回転していてもコードが読み取れることも確認します。
...
[File] img-qrcode.html
 ...
 39      let w = imgElemQr.width;
 40      let h = imgElemQr.height;
 41      ctx.drawImage(imgElemQr, 0, 0, w, h, -w/2, -h/2, w, h);
 42    }
 43
 ...
 56      mv.push_back(mat);
 57      cv.polylines(img, mv, true, new cv.Scalar(255, 0, 0), 5);
 ...
 64      let data = ctx.getImageData(0, 0, canvasElem1.width, canvasElem1.height);
 65      let src = cv.matFromImageData(data);
 66
...
壁紙とサンプルQRコードは`<img>`で読み込みますが（13～14行目）、どちらもCSSの`display: none;`から非表示にしています。これらは`<canvas>`に重ねて貼り付けることで可視にします（15行目）。壁紙を背景にQRコードを回転させているのが`rotate()`関数（33～42行目）です。壁紙とQRコードの画像中心を`CanvasRenderingContext2D.translate()`で合わせ、後者を`CanvasRenderingContext2D.rotate()`で回転しています。どちらも描画コンテクストの機能です。

回転は`setup()`関数（85～90行目）です。`setInterval()`で0.05秒に1回前記の`rotate()`を呼び出しています。360度×0.05秒/度なので、1周18秒です。
...
#### QR検出クラス－cv.QRCodeDetector

QRコードを検出・解読するには、まず`cv.QRCodeDetector`クラスをインスタンス化します（67行目）。

```javascript
 67      let detector = new cv.QRCodeDetector();
```

コンストラクタには引数はありません。戻り値はクラスインスタンスです。コンストラクタの定義を次に示します。
...
```Javascript
boolean cv.QRCodeDetector.detect(           // 真偽値を返す
    cv.Mat img,                             // 入力画像
    cv.Mat ponts,                           // 出力（座標値）
);
```

関数はQRコードが検出されたら`true`を、、失敗したら`false`を返します。
...
戻り値はUTF-8文字列です。第1引数`img`にはQRコードを含んだ画像を、第2引数`points`には`detect()`で取得した`cv.Mat`を指定します。

第3引数はバイナリコードを返しますが、オプションなので使いません。デフォルトの値が`noArray()`とありますが、これは行列指定がないという意味で、そういう関数がOpenCV.jsにあるわけではありません。

ここでは戻り値の文字列をそのまま`<span>`に書き込んでいます（20行目）。

#### 点の情報

`cv.QRCodeDetector.detect()`の返す`cv.Mat`は1行4列の2チャネル32ビット浮動小数点数（`cv.CV_32FC2`）で、次の図のようにチャネル0にx座標4つが、チャネル1にy座標4つがそれぞれ並んでいます。
...
第2引数`points`は、[5.4節](./05-colors.md#54-RGB画像を色成分に分解する "INTERNAL")で説明した複数の`cv.Mat`を収容するコンテナの`cv.MatVector`です（53行目）。複数といいつつ、ここでは要素は1つだけです。

要素に収容される`cv.Mat`のサイズは1行N列で、Nが点の数です。ここでは4点なので1行4列なので、サイズ的には`cv.QRCodeDetector.detect()`の戻り値と同じです。しかし、要素のデータ型は2チャネル32ビット符号あり整数（`cv.CV_32SC2`）でなければならないので、後述の型変換関数の`cv.Mat.convertTo()`で型変換をします。
...
OpenCVにはこれ以外にも`cv.Mat()`に幾何学図形を描く関数がいくつか用意されています。直線の`cv.line()`（[6.6節](./06-img.md#66-書類の傾きを補正する "INTERNAL")）、長方形（辺が画像辺と水平なもの）の`cv.rectangle()`（[6.7節](./06-img.md#67-顔を検出する "INTERNAL")）、円の`cv.circle()`（[7.4節](./07-video.md#74-ショットをトランジションでつなぐ "INTERNAL")）はのちほど取り上げます。その他の描画関数については、次のURLに示すリファレンスにまとめて記載されているので参考にしてください。

```https://docs.opencv.org/4.8.0/d6/d6e/group__imgproc__draw.html```

#### 型変換－cv.Mat.converTo

行列のデータ型変換をする`cv.Mat.convertTo()`関数を使います（54～55行目）。
...
`cv.Mat`インスタンスのメンバー関数なので、変換元のオブジェクトに作用させて使います。

第1引数`mat`には変換後の`cv.Mat`を指定します。例によって空のオブジェクトを用意します（54行目）。サイズやデータ型は関数が自動的に調整してくれます。

第2引数`rtype`には変換先のデータ型を指定しますが、このとき、入力とチャネル数が同じでなければなりません。たとえば、`cv.CV_8UC4`が入力なら、出力も4チャネルでなければならないので、`cv.CV_32FC4`などです。チャネル数が決まっているので、ビット深度（`cv.CV_32F`など）を用いてもかまいません。-1を指定すると、型変換はしません。

第3引数`alpha`と第4引数`beta`はピクセル値の定数倍と増減も用います。第3引数が指定されるとピクセル値にその値が掛けられるので、1より上の値なら輝度が増します。1未満なら暗くなります。第4引数も同様で、その値がピクセル値に加えられるので、正の値なら明るくなり、負なら暗くなります。オプション引数なので、指定がなければ輝度調整はありません。指定値にもよりますが、ピクセル値がデータ型の規定する最大値よりも大きく、または小さくなることもあります。その場合は強制的に最大値、または最小値に計算結果が抑えられます（飽和処理）。

[5.8節](./05-colors.md#58-背景を入れ替える輝度調整付き "INTERNAL")では、定数倍あるいは定数加算を計算するのに`cv.convertScaleAbs()`関数を代用しました。この`cv.Mat.convertTo()`関数も代用手段に使えます。`rtype`に-1を指定し、第3第4引数を使うだけです。
...
画像から電線や金網などの細線を消去します。たとえば、バックネットから撮影した球場のシーンからネットの線を抜くときに使います。

技術的には、モルフォロジー演算と呼ばれるテクニックを用います。OpenCVの関数は`cv.morphologyEx()`です。モルフォロジー演算は細線以外にも影響を与えるため、細線のなくなった画像からマスク画像を生成し、これらと元画像を合成します。この後処理は、[5.8節](./05-colors.md#58-背景を入れ替える輝度調整付き "INTERNAL")の`cv.threshold()`と`cv.Mat.copyTo()`の組み合わせ技と同じです。

実行例を次の画面に示します。

<img src="Images/Ch06/img-morph-1.png">

左上が元画像です。右上はモルフォロジー演算のなかでもクロージングと呼ばれるタイプの処理を適用したものです。水平に走る電線が消えましたが、メインの電柱と変圧器が水でにじんだ水彩画のように細部がつぶれています。
...
演算には膨張（dilate）、収縮（erode）、オープニング（opening）、クロージング（closing）の4つのタイプがあります。いずれも畳み込み演算と同じで、3×3などのカーネル（フィルタ）をすべてのピクセルに適用します。
...
左手が処理前の状態です。灰色の丸が注目ピクセル、そのカーネル（注目領域）は点線の枠線です。このとき、テトリスブロックもどきの前景がカーネルの左下に含まれているので、カーネル範囲をすべて白にします（中央）。この操作を画像全体に対して施すと、右手のように前景の輪郭が膨れ上がります。また、輪郭のでこぼこが減ります。

図にはありませんが、背景の黒がカーネルサイズ以上に広いところでは、カーネルに1点も白が含まれていないため、膨張操作をしてもまっくろが保たれます。逆にカーネルサイズよりも大きい白いエリアでは、その白がそのまま保たれます。
...
これを前述の収縮処理の模式図に当てはめると、白ピクセルはカーネルの左下に1つありますが、そこは構造要素では1ではありません。そこで、ここでは塗りつぶしはしません。
...
モルフォロジー変換については、次にURLを示すOpenCVチュートリアルの翻訳がていねいなのでそちらを参照してください。2016年から更新された気配がありませんが、技術に変わりはありません。
...
#### モルフォロジー操作－cv.morphologyEx

モルフォロジー演算のクロージング操作をしているのは41行目の`cv.morphologyEx()`関数です。
...
<!-- cv.getStructuringElement() はちゃんと使えます。第3引数はデフォルトのままなら指定しなくても大丈夫。-->
第4引数`kernel`には、`cv.Mat`形式でカーネルを指定します。カーネルの大きさは任意です。畳み込み演算と同じく奇数の正方形を用いるのが通例ですが、対象によっては、偶数長や長方形を使うこともあります。すべて1で埋めたものが一般的ですが、前述のように中央の行または列だけ1、十字型、星形、丸型など、求める効果に応じていろいろに設計できます。本節のように3×3程度なら手作業で作成してもかまいませんが、大きくて面倒そうなものには`cv.getStructuringElement()`というコンビニエンス関数が利用できます（詳しくはリファレンスを参照）。

第5引数`anchor`は[6.2節](#62-画像をぼかす "INTERNAL")の`cv.blur()`と同じく注目ピクセルのカーネル内での位置で、(-1, -1)は中央です。デフォルトのまま使うことが多いのですが、41行目では続く第6引数を指定したいので、デフォルト値であっても明示的に引数に指定しています。

第6引数`iteration`は処理の繰り返し回数です。`cv.MORPH_OPEN`でたとえば2を指定すると、収縮＞収縮＞膨張＞膨張と処理を繰り返します。1回だけでは効果が薄いときに指定します。41行目ではデフォルト1を指定していますが、他の値も試してください。

第7引数`borderType`も[6.2節](#62-画像をぼかす "INTERNAL")の`cv.blur()`と同じもので、画像端でのピクセルの外挿方法を定数から指定します。デフォルトは`cv.BORDER_CONSTANT`です。

第8引数`borderValue`は第6引数が`cv.BORDER_CONSTNAT`のときの固定値を指定します。リファレンスは、関数定義に示されている関数から得られるデフォルト値には特別な意味があると書いています。その値は、コンソールから確認すると次の値です（64ビット浮動小数点数の最大値）。
...
モルフォロジー演算を施した画像が得られれば、あとはこれに元画像を貼り付けるだけです（45～50行目）。元画像**に**貼り付ける（`morph.copyTo(src)`）ではなく、元画像**を**貼り付ける（`src.copyTo(morph)`）なところに注意してください。`cv.threshold()`と`cv.Mat.copyTo()`はすでに取り上げたので、説明は割愛します。
...
斜めにスキャンした書類をまっすぐにします。ヒトが読むぶんには少しくらい曲がっててもよいですが、あとでOCRなどで文字を読ませたいときには、やはりまっすぐのほうが精度は高まります。
...
㋐元画像は2つ用意します。1つは縮小表示した画面左のものです。縮小しているのは、A4サイズなど大きな書類をページに表示すると見にくいからです。もう1つは非表示（`display: none;`）です。これはオリジナルのサイズのまま`cv.Mat`に読み込むためのものです。縮小版だと回転角度の計算で精度が出ないからです。計算はすべてこの非表示画像から行いますが、残りの3つの画像からわかるように、表示時には縮小します。これには[6.1節](#61-一部領域をモザイク化する "INTERNAL")の`cv.resize()`を使います。
...
㋖平均角度で元画像を回転します。回転にはアフィン変換関数の`cv.WarpAffine()`を使います。アフィン変換には変換行列が必要ですが、回転用は`cv.getRotationMatrix2D()`で簡単に取得できます。
...
[File] img-hough.html
 ...
 50      }
 51
 52      console.log(`Hough mat format:
 53        Type=${mat.type()}, Cols=${mat.cols}, Rows=${mat.rows}`);
 54      mat.delete();
 55      return lines;
 56    }
 57
 58    function screenLines(lines) {
 59      let maxRadian = 20 * Math.PI / 180;
 60      let screened = lines.filter(function(arr) {
 61          let theta = Math.atan((arr[3] - arr[1]) / (arr[2] - arr[0]));
 62          return Math.abs(theta) < maxRadian;
 63      });
 64
 65      console.log(`After screening: ${screened.length}`);
 66      return screened;
 67    }
 68
 69    function drawLines(img, lines) {
 70      let color = new cv.Scalar(128, 128, 128);
 71      lines.forEach(function(line) {
 72        let p0 = new cv.Point(line[0], line[1]);
 73        let p1 = new cv.Point(line[2], line[3]);
 74        cv.line(img, p0, p1, color, 3);
 75      });
 76    }
 77
 78    function averageAngle(lines) {
 79      let radianArr = lines.map(function(arr) {
 80        return Math.atan((arr[3] - arr[1]) / (arr[2] - arr[0]));
 81      });
 82      let radian = radianArr.reduce((a, b) => a + b) / radianArr.length;
 83      let degree = radian * 180 / Math.PI;
 84      console.log(`Rotaion: ${radian} (${degree})`);
 85      return [radian, degree];
 86    }
 87
 88    function imgProc() {
 89      let src = cv.imread(imgElemHidden);
 90      cv.cvtColor(src, src, cv.COLOR_RGBA2RGB);
 91
 92      let contour = new cv.Mat();
 93      prepare(src, contour);
 94      showResized('canvasTag1', contour);
 95
 96      let lines = getLines(contour);
 97      let screened = screenLines(lines);
 98      let linedSrc = src.clone();
 99      drawLines(linedSrc, screened);
100      showResized('canvasTag2', linedSrc);
101      let [radian, degree] = averageAngle(screened);
102
103      let point = new cv.Point(src.rows/2, src.cols/2);
104      let rotMat = cv.getRotationMatrix2D(point, degree, 1);
105      cv.warpAffine(src, src, rotMat, new cv.Size(src.cols, src.rows));
106      showResized('canvasTag3', src);
107
108      [src, contour, linedSrc].forEach(m => m.delete());
109    }
110
111    var Module = {
112      onRuntimeInitialized: imgProc
113    }
114  </script>
115
116  </body>
...
処理は13行目の画像を対象に行うので、結果のサイズもそれと同じです。そのまま`cv.imshow()`でキャンバスに貼り付けると大きいまま表示されるので、表示前に`cv.resize()`でリサイズしてから表示します。この操作をやっているのが`showResized()`関数（25～31行目）です。
...
第1引数`rows`と第2引数`cols`の2点は`cv.Size`に置き換えることもできます（2引数になる）。データ型に制約はありませんが、1で埋められるのは最初のチャネルだけで、残りは0がセットされます。つまり、3チャネルのRGBにこの関数を適用すると、赤い画像が生成されます。
...
使い終わった`cv.Mat`は、解放するのを忘れないように（39行目）。

#### ㋒直線検出－cv.HoughTransformP関数

直線検出を行っているのが、42～50行目の`getLines()`関数です（呼び出しは96行目）。44行目が本節の主題のハフ変換です。
...
第2引数`lines`は結果の2点の座標値を`cv.Mat`として受け付けます。線分は複数あるので、4つの値が並びます（構造は後述）。ビット深度は`cv.32U`（32ビット符号あり整数）です。
...
第2引数`lines`から得られる`cv.Mat`のデータ型は4チャネル32ビット符号あり整数（`cv.CV_32SC4`）です。行数は検出した線分の数ぶんだけで、列数は常に1です。2つの端点座標(x1, y1, x2, y2)はそれぞれチャネルに収容されます。たとえば、y1ならチャネル1です。行列の構成を次に示します。

<!-- ODG に原画あり。565 x 358 -->
<img src="Images/Ch06/img-hough-lines.png" width="300">

`cv.CV_32S`を使っているので`cv.Mat.intPtr()`からデータを取得します（[4.3節](./04-mat.md#43-ピクセルの色名を判定する "INTERNAL")参照）。得られるのは4要素分の`Int32Array`です。これは、あとで使いやすいように4要素を配列の配列に直しています。この作業は46～50行目で行っています。
...
確認のために、`cv.Mat`のデータ型、行数と列数をコンソールに出力させています（52～53行目）。出力を次に示します。
...
データ型28番は`cv.CV_32SC4`です。行数が54個は少なめですが、`cv.HoughTransformP()`の第5～第7引数の閾値を低めに取るともっと出てきます（千個以上）。ここでは第5引数`threshold`で100ピクセル長以下を棄却せよと指定しているので、かなり絞られています。

#### ㋓直線のふるいがけ

上記で得た線分データを`screenLines()`関数からさらにふるいにかけます（58～67行目）。
...
ここでは絶対値にして20度（ラジアンなら0.349くらい）以上の角度の線は廃棄しています（59行目）。角度計算には`Math.atan()`を、指定の条件を満たす要素だけを取り出すのには`Array.filter()`が使えます。線分の長さによりフィルタリングはハフ関数側でやっているので、ここでは必要ありません。

ふるいにかけたあとの線分の数はコンソール出力から確認できます（65行目）。
...
形式は[6.4節](#64-QRコードを読む "INTERNAL")で取り上げた`cv.polyline()`とほぼ変わりません。違いは第2引数`pt1`と第3引数`pt2`で、端点2つの座標を`cv.Point`から指定します。ここではわかりやすいように、太さ3（第5引数の`thickness`）を指定しています。

#### ㋕平均角度の計算

線分からその回転角の平均を計算します（78～86行目）。線分座標を角度に置き換えた配列を用意し（79行目）、その平均を取っているだけです（82行目）。
...
<!-- Mat 使ってないので、これはいらない： OpenCVには`cv.Mat`のピクセル値の平均を計算する`cv.mean()`、平均と標準偏差を一気に計算する`cv.meanStdDev()`という関数もあるので、興味のある方は試してください。 -->

#### ㋖画像回転－cv.WarpAffine変換

最後に、元画像を回転します。

OpenCVには`cv.rotate()`といういかにもな名称の関数がありますが、これは90度単位でしか回転してくれません。そこで、回転はおろか、拡大縮小、平行移動、せん断（左右端を持って上下反対方向に動かすこと菱形状にする）をまとめて処理できる`cv.WarpAffine()`関数を使います（105行目）。

```javascript
105      cv.warpAffine(src, src, rotMat, new cv.Size(src.cols, src.rows));
```

4つの操作は、ある位置にあるピクセルを一定の方法で別の位置に移動するという意味では、どれも同じです。そして、これら「一定の方法」は次に示す同じ形式の行列式でまとめられるという点で数学的には等価です。これをアフィン変換（affine）といいます。数式で書けば次の格好です。
...
この式は、右辺にある元画像の $(x, y)$ にあるピクセルが回転等の操作によって別の位置 $(x', y')$ に移動することを示しています。 $(a, b, d, e)$ が拡大縮小、回転、せん断を、 $(c, f)$ が平行移動を担当しています。
...
cv.warpAffine(                              // 戻り値なし
    ...
    cv.Scalar borderValue = [0, 0, 0, 0]    // 境界値
);
```

第1引数`src`は入力の、第2引数`dst`は入力と同じデータ型の`cv.Mat`です。

第3引数`M`が上記のアフィン変換行列です。式では3×3ですが、最下端の行が常に同じ値なのでこれらを省き、2行3列の6要素で指定します。

第4引数`dsize`から出力画像のサイズを指定します。回転すると今までの枠内から角がはみ出すからです。ここでは元画像と同じサイズにしているので、4隅の一部が欠けます。

以降はオプションです。第5引数`flags`の補間方法は[6.1節](#61-一部領域をモザイク化する "INTERNAL")の`cv.resize()`と、第6引数`borderMode`の外挿方法は[6.2節](#62-画像をぼかす "INTERNAL")の`cv.blur()`と、第7引数`borderValue`は[6.4節](#64-QRコードを読む "INTERNAL")の`cv.morphologyEx()`のものとそれぞれ同じ意味です。

#### アフィン変換行列の生成－cv.getRotationMatrix2D

アフィン変換の第3引数は手で計算してもよいのですが、回転させるだけなら行列を回転角と回転中心座標から計算してくれるコンビニエンス関数が用意されています。104行目で使っている`cv.getRotationMatrix2D()`関数です。
...
関数はデータ型が`cv.CV_64FC1`（1チャネル64ビット浮動小数点数）で、2行3列の`cv.Mat`を返します。第1引数`center`には回転の中心座標を指定します。ここでは、画像の中心を指定しています（103行目）。
...
わかりやすいところで関数を試してみます。回転中心は(0, 0)、角度は60度、倍率は1倍（リサイズなし）でコンソールから実行します（出力は手で整形しています）。
...
技術的な主題は「Haar特徴をもとにしたカスケード分類器」と呼ばれる機械学習技術の利用です（以下「カスケード分類器」）。顔検出にはいろいろな手法がありますが、この方法はかなり高速です。加えて、カスケード分類器のパフォーマンスを最大限に引き出すため、ヒストグラム均等化と呼ばれるコントラスト調整も行います。

利用は本来なら簡単なのですが、外部から訓練済みデータファイルを読まなければならず、ブラウザ環境ではこれがやや複雑です。具体的には、`XMLHttpRequest()`というHTTPアクセスをする関数を用います。
...
[File] img-haarLike.html
 ...
 25        cv.rectangle(mat, point1, point2, new cv.Scalar(255, 255, 255), 3);
 26      }
 27    }
 28
 29    function imgProc() {
 ...
 43          ${req.status}, Type ${req.responseType}, State ${req.readyState}`);
 44
 45        let path = 'boo';
...
顔検出という複雑なことをしているわりに、コードは短めです。しかも、大半はデータファイルの読み込みと矩形描画のためのもので、エッセンスは35、51、54行目の3行だけです。

#### カスケード分類の原理

画像中に顔などの物体を見出すには、その対象の持つ特徴を明らかにしなければなりません。ここでは顔を対象としているので、部分領域の明暗差に着目します。というのも、正面を向いた顔の領域では、目のくぼんだあたりが暗く、頬のあたりが明るくなるように、明暗差が明らかだからです。

具体的には、畳み込み演算やモルフォロジー演算のように注目領域を設定します。そして、その領域を上下2つに分け、上と下のそれぞれのピクセル輝度の総和の差分を取ったものを特徴とします。この矩形領域の明暗差にもとづいた特徴を、Haar特徴と言います（ちなみに名称はAlfréd Haarから来ています）。注目領域の分割方法は上下だけでなく左右もあり、2分割だけでなく3分割4分割などもあり、正面顔以外のいろいろなものも対象にすることができます（顔以外ではナンバープレートの検出によく用いられるようです）。参考までにこれらのパターンを次図に示します。
...
この説明はかなり端折っています。正確なところを知りたいなら、次にURLを示すOpenCVチュートリアルを参照してください。
...
猫顔検出データファイルは、murtazahassanという方が作成されたもので、彼のGithubの「OpenCV-Python-Tutorials-and-Projects」リポジトリに収容されています。次のGithubページからダウンロードできます。
...
本書に収録したヒト用のデータファイルは正面顔用のものだけですが、OpenCVの配布パッケージには笑顔、両目、左目、右目、メガネ付き目、正面顔（複数あり）、全身、上半身、下半身、ナンバープレートに特化したもの、計17個が用意されています。OpenCVのドキュメントファイルには同梱されていないので、別途、次のGithubからダウンロードしてください。
...
均等化アルゴリズムはこの関数のリファレンスに説明されています。

#### 外部ファイルを読む－XMLHttpRequest

外部リソースであるカスケード分類器のデータファイルを読むには、JavaSctiptに標準で備わっている`XMLHttpRequest`クラスを用います。その名が示すように、HTTPリクエストを発することで外部からデータを取得するためのものです。ブラウザの中でブラウザがもう1つ動作するようなイメージです。対象となるデータはXMLにかぎらず、URLで取得できるものならなんでもかまいません。
...
得られたオブジェクトのメンバー関数`XMLHttpRequest.open()`からHTTP GETリクエストを準備し、発信します（39～62行目）。
...
用意ができたら`open()`でコネクションの準備をします。関数の第1引数にはHTTPのメソッド（GETやPUTなど）を、第2引数にはURLを指定します。URLは`http://...`で始まるフルのものでもよいですが、ここでは現在のページからの相対パスで指定しています（17行目）。オプションの第3引数では非同期モード（`async`）を使うかを指示します。`true`なら非同期モード、`false`なら同期モードです。同期モードでは受信データ形式を変更できないので、ここでは非同期を選択しています。
...
#### データを仮想ファイルに落とす－cv.FS_createDataFile
...
受信データは、OpenCV.js内部の仮想ファイルに落とします。というのも、後段のカスケード分類器のデータファイルを読む関数は、URLや`blob`データではなく、ローカルファイルのみを対象としているからです。仮想的というのは、スクリプト内でファイルのようにアクセスはできるものの、疑似的なもの（メモリ上のもの）なので、OSのファイルシステム上には存在しないという意味です。
...
先頭に`cv`があるのでOpenCV.jsの関数ですが、C++/Python版のOpenCVには存在しません（リファレンスにも記載はありません）。このファイル操作のメカニズムは、下位で用いられているemscriptenが提供しているものだからです。関数の定義を次に示します。

<!-- FunctionDefinition cv.FS_createDataFile() データから仮想ファイルを作成する。 -->
```Javascript
cv.FS_createDataFile(                       // 戻り値なし
    string parent,                          // 親ディレクトリ名
    string name,                            // ファイル名
...
第3引数`data`には仮想ファイルに描き込むデータを`Uint8Array`形式で指定します。受信した`ArrayBiffer`を`Uint8Array`コンストラクタで変換しているのはこれが理由です。
...
#### データファイルを読む－cv.CascadeClassifier
...
コンストラクタからインスタンスを生成し、その関数の`load()`でファイルを読み込むだけです。
...
関数は読み込み成功なら`true`を、失敗なら`false`をそれぞれ返します。第1引数`filename`で指定するファイルパスは、`cv.FS_createDataFile()`の第2引数で指定したものです。

#### 検出－cv.CascadeClassifier.detectMultiScale
...
第2引数`objects`には矩形を表現する`cv.Rect`（四角形）を収容する`cv.RectVector`を指定します（53行目）。`cv.RectVector`は複数の`cv.Rect`を収容するコンテナ（配列）で、その用法は複数の`cv.Mat`を収容する`cv.MatVector`と同じです（C++ならどちらも同じ`vector`で表現されます）。i番目の要素を取得するのに`.get(i)`関数を使うのも同じです。
...
用法や引数は、`cv.line()`関数（[6.6節](#66-書類の傾きを補正する "INTERNAL")）と同じです。
...
画像合成は、Adobe Photoshopなどの画像編集アプリケーションでは、オーバーラップしたレイヤーを透かして見せる操作です。透けたように見えるのは透明度（アルファチャネル）の機能ではなく、RGB値の四則演算の組み合わせによるものです。本節の目的は、こうした単純な演算で画像がどう変化するかを確かめるところにあります。
...
加算は、2枚の画像を透明なシートに印画し、重ねたうえですかして見たような絵を生成します。透明シートと異なるのは、もともと明るいところ同士を重ねると値が飽和し、白く飛んでしまうところです。ここでは、もともと明るい左の背景が飽和しています。その代わり、ダックスフンド部分はもとが黒いので、そのテクスチャが明るいほうの風紋に置き換わります。このサンプルのように背景と前景がはっきりした画像なら、プレインな一色の画像を加えることで、着色ができます。
...
第5引数`dtype`は出力のビット深度を指定するためのものです。`cv.add()`では足す数と足される数のビット深度（ビット数）は同じでなくてもよいので、たとえば16ビット符号ありと8ビット符号なしを足して、32ビット浮動小数点数で保存することもできます。ただ、できるというだけで、そういう妙なことをしなければならないのは、特殊な事情でしょう。普通は入力の2枚の画像はデータ型を揃え、出力もそれと同じになるよう、デフォルトの-1を指定します。
...
```javascript
 41      cv.subtract(img2, img1, result);
 42      result.convertTo(result, -1, 1.2, 20);
```
...
`cv.Mat`同士の演算には四則演算以外にもビット演算（ANDやOR）、べき乗、最大最小値などいろいろな関数がで定義されているので、それらでどんな絵が得られるかを試すのも楽しいでしょう。次のURLに示すリファレンスの「Operations on arrays」にまとめて掲載されていますので、詳細はそちらを参照してください。
